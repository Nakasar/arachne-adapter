...
exports = module.exports = class Adapter {
    constructor({ brainUrl = "http://localhost", token = "", mongoURL = "mongodb://localhost:27017/arachne-adapter", admins = [], sendMessage = () => { } } = {}) {
    ...
    start() {
        database.connect(this.mongoURL).then(db => {
            ...
            this.configurationManager.init().then(() => this.configurationManager.getConfig()).then(config => {
    ...
    handleAdmin({ command, data }) {
        return Promise.resolve().then(() => {
            ...
            switch (cmd) {
                ...
                default:
                    return {
                        text: "Unkown commad. Type `help` to get list of administrative commands.",
                        private: true,
                        privateName: data.userName
                    };
            }
    ...
    handleMessage({ type, text, room, username, data }) {
        return Promise.resolve().then(() => {
            ...
            if (type === 'admin') {
                ...
                return this.handleAdmin({ command: text, data: sentData });
            }
            ...
            return this.threadManager.checkThread(room).then(threadId => {
                if (threadId) {
                    ...
                    switch (type) {
                        ...
                        default:
                            throw new Error({ message: "Unrecognized type. Can only handle messages of type sentence and command.", id: "UNKOWN_MESSAGE_TYPE" });
                    }
                }
            })
        }).then(response => {
