...
exports = module.exports = class ConfigurationManager {
     ...
     */
    constructor(database, config,updateToken,updateUrl) {
    ...
    persistToken(token) {
        ...
        return this.configCollection.updateOne({}, {
            $set: {
                token
            }
        });
    ...
    removeAdmin(admin) {
        return Promise.resolve().then(() => {
            if (this.config.admins.length === 1) {
                throw new Error({
                    message: "Cannot delete last admin user (yourself).",
                    id: "ONE_AMIN_REMAINING"
                });
            }
