...
exports = module.exports = class HookManager {
    ...
    init() {
        ...
        this.collection.find({}).toArray().then(hooks => {
            hooks.map((hook) => {
                this._hooks.set(hook.key, { room: hook.room, skill: hook.skill, messageOnDelete: hook.messageOnDelete });
            });
    ...
    createHook(hook, room) {
        return Promise.resolve().then(() => {
            ...
            this._hooks.set(hook._id, { room, skill: hook.skill, messageOnDelete: hook.messageOnDelete });

            return this.collection.insertOne({ key: hook._id, room: room, skill: hook.skill, messageOnDelete: hook.messageOnDelete })
                .then(created => {
    ...
    removeHook(hookId) {
        return new Promise((resolve, reject) => {
            ...
            this.onCloseHook({ hookId, room: hook.room });

            // Delete from the locals hooks
            this._hooks.delete(hookId);

            // delete from the DB
            this.collection.deleteOne({ key: hookId }, (err) => {
                if (err) {
